...
v0.6 加入焦點話題、報導基調、關鍵字追蹤、新報導標記與近 24 小時風向分段。
...
不同事件、後續發展與各方反應；右側顯示媒體家數與報導則數。點家數按鈕只看該話題，
再點一次或「取消話題篩選」恢復。進入話題會清除來源、類別與題材篩選；手動改來源或
類別也會取消話題篩選。補送保留仍存在的話題選擇。沒有話題時，焦點改列至少三家媒體
同報的事件，點家數可展開清單中的其他報導。

話題中至少五則報導已判斷基調時，標題下方會出現細條與「報導基調」計數：正面、負面、
中性、正反並陳。這裡按報導計數，呈現標題與摘要對事情的評價，不代表民意或事情的真相，
也不是股市多空。話題判斷與基調沿用同一 API key，只送標題＋摘要，資料出境規則同前節。

離開頁面或卸載模組時，會記下本次列表最新的發布時間。下次開啟後，比上次基準新的報導
會標「新」；同事件只要有一則新報導，代表列就會標記。狀態列的「N 則新」按目前清單的
事件數計算，不是已讀／未讀紀錄；第一次使用沒有基準時不標記。跨日報導顯示日期，
當地時間剛好為 00:00 的報導只顯示「今天」或日期，避免把僅有日期的來源當成午夜發布。

點工具列「追蹤」可設定關鍵字，以空白或逗號分隔，按「儲存」或 Enter 套用。最多十個，
每個 1～20 字，不分大小寫去重；標題或摘要包含關鍵字就符合，特殊符號也照字面比對。
事件內任一符合條件的報導命中，代表列會顯示「追蹤：關鍵字」。開啟「只看追蹤」可再
縮小清單，與其他篩選並用；按鈕與狀態列的追蹤數都按事件計算。沒有關鍵字時無法開啟，
空清單的「清除篩選」也會關掉只看追蹤。
...
- `modudock.module.news.lastSeen`：上次列表的發布時間基準，供「新」標記使用。
