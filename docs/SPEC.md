...
## 19. 現行行為總覽（v0.8 來源增補見 §21）

本節以第 46 輪收件後的程式（`452628d`）建立，於 §20.18 核對至 `a00347c`，再於 §20.43 依 `9927ec8` 逐段核對（涵蓋 §20.1～§20.42），是閱讀目前行為的入口。§18 保留演進過程；若舊文與本節不同，以本節描述的現行實作為準。API 題目與 criteria 的逐字定義仍見 §12.2／§12.5、§13.3、§16.2、§17.1、§18.5／§18.8／§18.16；本節不重新改寫題目。下列出處同時標示必要的早期基礎規格，避免誤把所有功能都歸於 §18。
...
- **解析與列表名額**：RSS／Atom 正規化 title、link、published、summary、source、time_guessed；標題最多 300 字、摘要 200 字，原始欄位先截 8192 字元再以線性掃描去 HTML 標籤，不再交 HTMLParser；不完整片段只在原處處理，不把整欄退回 HTML；截斷屬性尾標籤丟棄、非標籤的 < 留作文字、未知 <![…]> 宣告丟棄（§20.6）。僅收可解析的 HTTP(S) 連結，相對路徑依最後 feed URL 解開。合併依 dedup key 去重：同來源取較新報導；跨來源優先選連結主機屬於該來源受信任網域者，再按 feeds.json 順序，發布時間不能跨來源搶所有權。去重後每來源先保留最新 min(20, 則數, 該來源上限)，餘額按時間填入，一般來源最多 60 則，自由時報最多 120 則，最後仍新到舊排序（§21）。`MAX_ITEMS_LIST=550` 是列表與一般模型佇列共同上限；保底名額不能突破總上限，封包守衛仍可裁切。（出處：§5.6、§13.2、§18.1、§18.33、§18.50、§20.2）
- **v0.8 來源保留**：現有16來源；自由時報可保留RSS漏列但仍在三小時內的歷史報導，當前RSS不因三小時過期、總cache最多120則。200更新當前成員身分；304／失敗不續期歷史報導，每輪收尾清除過期漏列項目，first_seen仍獨立1000筆LRU。只有本機記憶體，不補抓啟動前漏文；其他來源快取不變。可選retain_hours／max_items與預檢詳見§21.3。
...
- **話題擴展與穩定身分**：上一份已送出列表仍存在的 seed key 優先嘗試，但不保證入選名額。候選與種子事件最新時間相差≤48h，且與目前成員共用特徵詞；種子事件成員照舊直接納入；其他候選 true 時，若其完整事件達三家 outlet，整事件尚未 claimed 的報導一起納入；未達三家的事件只納入自身 true 的報導（§20.39）。小事件已有成員的同事件兄弟篇可不共用詞，但仍須在 48h 窗內，各自查答案或排待問，並優先於一般候選。每次 plan、每種子最多列 60 個待問候選，同優先層依共用詞多、時間新、key 排序（§20.36）；答案回來可再 plan。每批一個種子＋最多 19 候選、≤8000 字，same_topic 且 p_max≥0.7 採納。最多建 10 個話題，再按家數、則數、id 取 5 個，僅保留入選者待問；話題 id 是 seed key 的 SHA-1 前 12 碼。previous 種子擴展後仍須至少三家。 claimed 按實際成員 key 排除；只有不足三家的小事件可能部分被收，剩餘報導可參與其他話題，但不具新種子資格。達三家的大事件始終整組納入或尚未納入，不會留下部分報導另建種子；未被 claimed 的 previous seed 保持精確身分，擴展後仍須三家（§20.39、§20.42）。（出處：§18.5、§18.7、§18.12、§18.18）
...
- **快取與有界工作**：分類／分析／基調快取各 4000、配對／話題快取各 20000，皆 FIFO、只存成功結果（配對與話題的 false 也是成功判斷）。一般工作佇列各 550、配對 1100（§21）；各類 in-flight 同時涵蓋排隊與已取走工作，避免重問，完成／失敗均釋放。配對結果接收後會補排溢出的候選。來源快取、模型快取、分群與列表由協調者管理；worker 只回候選，results 上限 32。（出處：§12.3、§13.4、§16.4、§18.5、§18.8、§18.32、§18.37）
...
- **預算與跨輪接手**：一輪模型工作從第一批准入起共用 60 秒准入預算；在途請求可超過此時點完成。一般失敗停止該 work 的新准入；暫時性回應驗證、連線／逾時與一般5xx失敗依 §20.46 保留原批，在其他可執行工作後於原 deadline 內補試一次，成功不記failed，補試失敗才停止。busy耗盡、401／403與熔斷探測不補試；其他在途成功仍可採納；晚到模型結果不因舊輪 id 而一律丟棄。新輪若先因舊輪 in-flight 跳過某 key，舊結果回來／失敗後會在目前 work 仍可准入且有 last_list 的條件下重排未完成工作；active 抓取期間不補送列表，下一次 emit 帶快取結果。（出處：§12.6、§18.2、§18.32、§18.37）
...
- **暫停原因說明**：短字仍為「整理暫停，下次更新繼續」；title／describedby 按 failure 提供建議：busy 為服務忙碌、稍後自動重試；connection 為連線失敗、檢查網路；response 為回應無法使用、持續發生請聯絡維護者；service 為「模型服務暫時無法使用，下次更新自動重試」；未知代碼用一般整理未完成說明。budget 與 waiting 分別說明預算或等待接手；不把遠端模型錯誤本文送前半（§20.21、§20.40）。
...
- **焦點入口**：優先最多五個話題；未處於話題範圍時，來源＋類別只決定話題是否有符合成員；話題範圍包含 selectedTopic 與 selectedCount.topic，兩者均保留所有話題入口，內部切類別不隱藏這些入口。收合標頭的話題數與展開列數一致，目前話題入口 aria-pressed=true，再按一次返回原檢視，不清空後重進話題（§20.44）。題材／數字／搜尋／新進展不篩話題卡，話題家數、基調與新增數仍按整個話題。被過濾光不回退事件焦點。寬窄版皆以「看話題・N 家」進話題（§20.24），標題連結仍開原文；話題 title、最新不同事件的報導標題、基調、上次之後新增事件數依資料呈現。最新列與種子同標題或同事件即不重複列，長標題單行省略、title 留全文。（出處：§18.6、§18.20、§18.23、§18.39、§18.43）
...
- **標為已看／復原**：有 lastSeen 且目前篩選下新增數>0 時顯示「標為已看」；復原中隱藏標記鈕。基準取 min(max(完整目前列表最新 published, body.at), Date.now())；候選無效或不比現基準新則禁用。手動立即寫 lastSeen，重算新進展、分隔線與徽章；容許的未來報導仍可能算新，不保證歸零。復原一次回到操作前基準（可為 null），下一份有效 list 到來即取消復原，不分同／新 at，沒有十秒計時器。手動操作最後寫入為準、不同頁不即時同步；離開時仍保留讀取較新儲存值的保護（§20.21）。
- **話題進出**：首次進入保存來源、類別、題材、數字篩選、追蹤與新進展開關、最近可捲動祖先位置與焦點身分，再清空來源／類別／題材／數字、關閉只看追蹤；保留搜尋字與當時的新進展開關。切另一話題不覆寫保存；返回原檢視、再點同話題、話題消失都還原有效項目；手動改來源或空清單清除則丟棄原檢視；話題內手動改類別保留話題及返回狀態，清掉題材／數字，面板按話題×類別計算；此類別切換是臨時檢視，不寫 localStorage，返回原檢視後把實際還原的 source／category 寫回 view（§20.14）。話題內數字改點題材會保留話題與返回狀態（§20.6）。話題篩選列另按 outlet 合併為各家報導語氣對照，預設五家、同數按該 outlet 最早的 feed 順序；「另 N 家（共 M 家）」可展開全部，再收合；換話題重設、同話題補送保留，title 列全部媒體與報導數（§20.25、§20.32）。來源下拉仍可分別選各 feed。（家數修正見 §20.4 R4-A）（出處：§18.6、§18.15、§18.21、§18.25、§18.43）
...
- **首次閱讀階段**：up 後仍為「等待新聞更新」，第一份 list 若 working 顯示「新聞已可閱讀・整理分類與話題中」；由首份 working 起滿 30 秒仍 working 且未在重新整理中，才加「可按重新整理」，不估完成秒數、不依補送才更新。按下重新整理立即隱藏此建議，同 at 補送不重設起算時間；新 at 到達或首次離開 working 即結束首載階段，取消計時器。完成時消失，paused／off 沿既有原因提示，後續輪 working 仍用「整理中」，不重啟首載建議。計時器於結束及 unmount 清除。（出處：§20.28、§20.44）
...
| §18.4／§18.13／§18.20、§18.6／§18.21／§18.23 | 舊文仍見「N 則新」「上次看到這裡」「返回」「N 家媒體・M 則」。現行依 §18.43 改為新增 N 個事件、以下為上次離開前的新聞、返回原檢視、看話題・N 家／看同事件・N 家；窄版話題入口依 §20.24 同樣保留「看話題」。 | `front/front.js:drawItems`、`drawFocus`、`drawPanel` |
...
| §18.20 off=disabled | 實際依 §18.44 回 no_key／auth；大小預留依 §20.21／§20.40 改取 paused/failed 與允許 failure 代碼的最長序列化形狀（connection），不再用 disabled。 | `back/scheduler.py:_model_state`；`back/feedparse.py:MODEL_RESERVE` |
...
每輪一行，描述收件結果；早期行為若被後輪取代，以 §19 現行行為為準。第10、22、31輪為變異驗證，第35、37輪為離線診斷／評估；部分輪沒有獨立長篇小節，不代表漏做。

| 輪次 | 做了什麼／為什麼 |
|---|---|
| §20.1 | 面板計數可點開對應事件、補焦點空狀態，讓數字可核對。 |
| §20.2 | 線性處理 feed HTML、保護跨來源去重所有權，阻止解析耗時及來源搶新聞。 |
| §20.3 | 穩定 chaos 話題覆蓋，標示失敗來源沿用資料的確認時間。 |
| §20.4 | 家數改按 outlet 去重；兩家種子實驗未增加有效話題，未採用。 |
| §20.5 | 加入臨時標題／摘要搜尋，與現有篩選交集並顯示命中報導。 |
| §20.6 | 修 HTML 截斷相容性、話題內類別／題材流程與搜尋回饋。 |
| §20.7 | 多報導事件增加最新報導連結，保留最早代表又能快速看進展。 |
| §20.8 | 股市文字改偏多／偏空並標明非行情，補搜尋統計範圍與色條圖例。 |
| §20.9 | 來源與類別改事件分面計數、縮小焦點空狀態並說清追蹤用途。 |
| §20.10 | 掃描第一批前後半邏輯變異，補測到零存活。 |
| §20.11 | 缺分析不算無關，加入只看新進展及面板範圍說明。 |
| §20.12 | 報導語氣數字可展開逐則核對，減輕按鈕並隱藏新增零值入口。 |
| §20.13 | 首頁加入跨類別新聞風向速覽，補主詞、件數及小樣本提醒。 |
| §20.14 | 修話題偏好保存、隱藏搜尋仍生效、新進展交集與搜尋展開殘留。 |
| §20.15 | 加入 ? 與快捷鍵入口，明訂 Esc 每次只收一層。 |
| §20.16 | 擴充前半隨機漫步及計數／焦點／保存不變式。 |
| §20.17 | 根建立定位參考，避免隱藏輔助文字撐出殼的第二條捲軸。 |
| §20.18 | 首次核對 §19／README 與程式，修文件並列程式缺口。 |
| §20.19 | 修追蹤計數、排行最大值縮放、話題須等配對完成的准入守衛。 |
| §20.20 | 分列實際 HTTP／重試量、全來源失敗提示；修 deferred awaiting 與候選重算。 |
| §20.21 | 加入標為已看及單次復原，暫停原因給出可行動建議。 |
| §20.22 | 掃描第11～21輪變異，31個首輪全殺，無新增程式。 |
| §20.23 | 精簡新進展／語氣字樣、強化入口、補大盤其餘無方向數。 |
| §20.24 | 窄版面板預設收合、訊號2×2、工具列兩行，縮短首屏。 |
| §20.25 | 恢復寬窄工具列的視覺與 Tab 順序，媒體剩餘家數改清楚說法。 |
| §20.26 | 話題加入時間順讀與按 outlet 篩選，方便比較發展及各家報導。 |
| §20.27 | 窄版加480／500遲滯避免捲軸振盪，並更新搜尋範圍提示。 |
| §20.28 | 補送保住滑鼠閱讀位置，首次載入說明新聞已可讀。 |
| §20.29 | 話題分列整體與目前顯示的事件／報導數，避免混淆統計範圍。 |
| §20.30 | 頂端補送不作捲動補償，讓工具列與新新聞留在前方。 |
| §20.31 | 掃描第25～30輪變異，補強排序、窄版與閱讀位置測試。 |
| §20.32 | 媒體分布升級各家語氣對照，補可展開家數與清除媒體 chip。 |
| §20.33 | 話題內收起焦點卡、明確兩種排序，混合語氣事件顯示組成。 |
| §20.34 | 改稱報導語氣／兩面，提醒非媒體立場、小樣本及待判定。 |
| §20.35 | 離線人工診斷話題成員路徑，定位同事件連帶誤入。 |
| §20.36 | 非種子事件先逐篇確認話題資格，減少無關兄弟篇污染語氣。 |
| §20.37 | 重跑基調評估低信心回退；0/117回退、分布相同，未新增判斷不明。 |
| §20.38 | 假 client 長跑驗資源平台期，加入200輪有界回歸。 |
| §20.39 | claimed 改逐 key；三家大事件整組納入，小事件保留逐篇確認。 |
| §20.40 | 403 改可恢復暫停與30分鐘熔斷，401維持永久關閉，補真實品質驗證。 |
| §20.41 | 每輪來源摘要列故障類型、慢來源及連敗，方便定位與統計。 |
| §20.42 | 修已 logged 工作探測閘門、晚到403計數，刪不可達的剩餘種子分支。 |
...
- **話題准入候選快取**（共用候選與有界副本快取已由§21.6擴充）：依 last_list 物件身分快取一份非自動配對 key 集合，不快取 unresolved 布林值；每批仍查最新 event_cache，所以答案提交與 FIFO 淘汰立即生效。同一列表多次檢查只呼叫一次 candidate_pairs，替換列表就重算，最多持有一份列表與候選。300 則離線 fixture 全部候選已解決時，本機中位數由重算 19.765ms（10 次）降為快取查詢 0.015ms（100 次）；不作硬體無關效能保證。
...
- **R28-2 首次階段**（重新整理中隱藏建議、新 at 結束階段的補充依 §20.44）：等待新聞更新→第一份 working list「新聞已可閱讀・整理分類與話題中」→完成時消失。由第一份 working 起 30 秒仍 working 加「可按重新整理」，單次 timer 到時即更新，補送不重設時限；不估秒數。首次非 working 結束此提示階段，paused／off 使用既有降級原因，後續輪回原短字「整理中」。計時與新增互動監聽在 unmount 清理。
...
- **成員**（大事件例外已由 §20.39 取代）：種子事件的所有報導仍直接納入。其他候選僅在 cache[(seed,key)] 為 true 時納入該 key，不再帶入整個 event；false 不會被同事件 true 覆蓋。話題 sources／count／keys 與語氣統計皆只計實際成員，不改前半事件分組或事件 id。
- **同事件待問**：已有成員所屬 event 的其他報導，不要求再次共用特徵詞，逐則查快取／加入待問；仍遵守種子最新時間 ±48h、已被先前話題收走的 key 排除（§20.39）、每 plan 每種子 MAX_PENDING=60、快取與每輪預算。兄弟篇優先一般詞候選，原因是先完成已出現事件的報導資格，避免各家語氣混入未確認成員；同層維持共用詞數、時間、key 排序。滿額未問留後續 plan，不額外開 worker 或延長預算。
...
### 20.39 第 39 輪：逐報導 claimed 與大事件整體納入

- **問題與折衷**：§20.36 的逐篇確認配上 event 級 claimed，會封鎖未收的兄弟篇；僅改 claimed 為 key 仍可能讓五家頭條被收一篇後剩四家、排名落到第六而消失。保留小事件逐篇確認的誤入改善，同時信任本身具種子資格的大事件為同一件事。
- **成員規則**：先按完整輸入事件的不同 outlet 數判斷是否達三家（不是 feed 數，也不是剩餘人數）。候選的 cache[(seed,key)] 為 true 時，達三家的事件整組尚未 claimed 的報導一起納入，包括未問或該 seed 下為 false 的兄弟篇；未達三家的事件只納入直接 true 的 key，兄弟篇各自排待問、true 才收。種子事件直接納入規則不變。
- **claimed／身分**：claimed 只記實際成員 key，候選及 sticky seed 排除都按 key。每個種子只直接納入其事件中未 claimed 的報導，話題間成員不重複。新種子從完整事件至少三家者選取最早（同時依來源順序、key）為代表；此類事件只會整組被 claimed，不存在部分剩餘三家另建種子的情況（§20.42 修正文義與程式分支）；未被 claimed 的精確 previous seed 沿用 id，擴展後仍須三家。false 僅作用於該 seed→key，不封鎖另一話題判斷；上述大事件整組納入為刻意例外。
- **待問／排名**：小事件兄弟篇仍優先一般候選，沿用 ±48h、每次 plan 每種子 60 個待問、快取與預算。最多建十個、按家數／則數／id 取前五不改。測試不增加名額或刪除競爭話題以迴避裁切。
...
### 20.42 第 42 輪：探測准入、晚到 403 與種子不變條件（jev 0）

- **R42-1 全域在途**：scheduler 在 cv 下維護所有模型呼叫的 model_running，准入加一、完成減一，包含已 logged 後又補件的 work。熔斷期間只要任何模型呼叫在途，其餘 worker 等待 cv；不再以 model_rounds（只負責未印統計者）判斷。成功探測清零後同輪可繼續，不改模型統計的一次性規則。
- **R42-2 成功輪序**：共用 client state 記錄最後成功回應的最大 round id；_request 在解碼成功後傳入該請求的原輪序，不能以當下最新准入輪代替。比成功輪序更舊的 403 不計連敗、不改 probe_at；同輪或較新的 403 仍按既有去重規則計。成功仍清零，成功輪序不倒退；只保留常數大小的整數與至多三個失敗輪 id。單獨呼叫（無 scheduler 輪序）保留既有恢復行為。
- **R42-3 話題種子**：修正 §19／§20.39 的「部分收走後剩餘三家可再當種子」。目前大事件（完整事件至少三家 outlet）一旦納入任何話題就整組納入，因此不可能部分剩餘；小事件雖可部分收走，但原本不足三家，剩餘更不可能達門檻。刪除剩餘家數重算與重選代表分支，直接使用完整大事件最早者為新種子、已 claimed 者跳過。小事件既有 previous seed 若未 claimed 仍可試擴展，最終話題仍須三家；claimed 仍逐 key，非種子小事件仍逐篇確認。
- **驗收**：兩支原 PoC 修前重現；repo 三條測試先紅後綠。logged work 測試用實際 worker 與阻塞假 HTTP server 建立在途（而非只手改 work.running），確認同輪40則最終全數分類、無 paused/service、全域在途歸零、無重印統計；晚到 403 另以假 server 驗證舊請求在新輪成功後返回，且新輪序不被其他更新的准入誤推。話題既有完整測試維持通過。

### 20.43 第 43 輪：第二次現行文件一致性核對（jev 0）

核對基準為 `9927ec8`，逐段檢查 §19 的後半、前半、驗證、限制與歷史索引，並對照 front 三檔、back 各 Python 檔及 real_run。以下均為文件舊文或缺漏，已在文件修正；本輪未確認新的程式違規，未改程式或測試。未修改逐字 API 題目與 criteria。下表同時記錄使用者向 README 的修訂，功能未另行擴張。

| 編號／位置 | 不一致或缺漏 → 處理方式 | 程式依據 |
|---|---|---|
| D1 §19 開頭／§19.5 標題 | 核對基準仍停第17輪 → 更新到第42輪收件。 | `9927ec8` |
| D2 §19.1 關閉與退避 | 「連續三輪」容易誤解為其他錯誤會斷連續 → 明寫自有效成功後累計不同403輪，其他錯誤不清零。 | `back/classify.py:153`、`:166` |
| D3 §19.1 關閉與退避 | 未納 R42 全域在途及成功輪序 → 補已 logged work 也受探測閘門保護、舊403不重計。 | `back/scheduler.py:393`、`back/classify.py:153` |
| D4 §19.1 關閉與退避 | 一般429/529退避敘述未排除熔斷探測 → 補探測只有一次HTTP，不重試。 | `back/classify.py:280` |
| D5 §19.2 狀態／閱讀基準 | 僅提標記／復原，缺完整基準與有效期 → 補全列表 published／at 取較新、上限現在、按鈕條件及下份 list 取消復原。 | `front/front.js:1329`、`:1636`、`:1732` |
| D6 §19.2 狀態 | 只有泛稱暫停建議 → 補 failure 五類對應與 service 可恢復說明。 | `front/front.js:1878` |
| D7 §19.2 話題進出 | 媒體仍像「最多五家」靜態文字 → 改預設五家、展開全部、跨話題重設與同話題保留。 | `front/front.js:1528`、`:1606`、`:1612` |
| D8 §19.2 話題工具 | 未述各家加總與跨媒體完整事件數可能不同，亦缺全待判定與窄版分組 → 補 title 的計數限制、空條欄位及群組間距。 | `front/front.js:212`、`:1517`、`:1583`；`front/style.js:187` |
| D9 §19.3 驗證／觀測 | 收件數停331／162，未提 sources 摘要及新耐久回歸 → 更新為384／513快照，補來源行、200輪測試與3000操作觀測及限制。 | `scripts/real_run.py:81`、`:183`；`tests/test_model_endurance.py`；§20.38 |
| D10 §19.5 歷史索引 | 仍寫窄版話題只剩N家、MODEL_RESERVE用disabled → 改現行話題入口及最長failure形狀。 | `front/front.js:749`；`back/feedparse.py:46` |
| D11 README 分類／模型運作 | 多處把403列永久關閉、清分析 → 改401才永久關閉；403保留快取分析並暫停／冷卻。 | `back/classify.py:246`、`back/scheduler.py:_decorate` |
| D12 README 財經／國際 | 已分析X／N像一直顯示 → 明寫只在未完成時顯示。 | `front/front.js:1044` |
| D13 README 焦點／閱讀工具 | 舊「報導基調」標題、「等N家」、「上次之後新增N事件」可見字 → 更新報導語氣、可展開全部與短標+N新事件；完整無障礙句保留。 | `front/front.js:657`、`:774`、`:1608` |
| D14 README 使用與限制 | 缺標為已看、順讀／媒體對照、窄版收合、閱讀保位、初載／全掛提示與新話題成員限制 → 加一屏內使用者摘要，限制說明另補。 | `front/front.js:390`、`:423`、`:1528`、`:1636`；`back/topics.py:54`～`:90` |
| D15 README 維運 | 只述批次數與model行 → 補http／retries／累計與sources行，real_run摘要一起說明。 | `back/scheduler.py:373`、`:825`；`scripts/real_run.py:183` |

另新增 §20.0 一張42輪總覽表；沒有獨立長篇小節的變異／離線評估輪也列入，未採用方案明確標示未採用。核對過但沒有新落差的 Esc 優先序、四個 localStorage key、分面計數、題材／數字互斥與話題範圍、480/500／560/540 遲滯、事件／話題門檻，不重複列成問題。文件檢查包含42行完整性、重複標題與 git diff --check；純文件改動不重跑模型或程式測試。


### 20.44 第 44 輪：話題數字範圍一致、首載提示與漫步資源釋放（jev 0）

- **R44-1 話題內數字篩選**：焦點的話題範圍統一取 selectedTopic 或 selectedCount.topic。範圍內保留全部話題入口，收合標頭的 N 與展開列數相同，不再因清單類別另外濾掉話題卡；目前話題入口維持 aria-pressed=true，再按直接返回原檢視。保留來源、類別、題材／數字與既有返回保存規則。p3 重現轉為真 click/change 測試；舊三條數字篩選返回測試的未按下／點兩次斷言依本輪要求改正，其原檢視還原斷言不減弱。
- **R44-2 首載生命週期**：重新整理進行中不顯示「可按重新整理」，點下立即重算文字；即使初載 timer 到時或同 at 補送也不重現。選擇「新 at 結束首載」而非重啟計時：它代表讀者已收到下一輪資料，後續只用原本「整理中」，避免一直把首載建議當成故障警告。同 at 仍沿用初始時限，首次非 working 也照舊結束。以假 clock 覆蓋提示已出現後刷新、忙碌中 timer／補送、新 at 取消尚未觸發的 timer 與晚到回呼。
- **R44-3 漫步 harness**：happy-dom 20.14.5 的 WindowBrowserContext 靜態 browserFrames Map 強參考每個 Window／frame；abort() 只取消非同步工作，不移除登錄，模組 unmount 無法代替測試視窗的 close()。不用 news 模組也可重現 Map 大小 0→建立 Window 1→abort 仍1→close 回0；不是模組全域快取洩漏。每 seed finally 在 unmount 後 await happyDOM.close()，即使 unmount 丟錯也關閉；一般前半測試的 setup 同樣完整卸載／關閉。每 seed 斷言 window.closed=true，改回 abort 的變異會紅。保留原動作池、seed 與所有行為不變式，不以縮小負載或提高 heap 上限迴避問題。
- **記憶體診斷**：`NEWS_WALK_MEMORY=1` 配合 `node --expose-gc --test tests/front.walk.test.mjs`，每100 seeds GC後記錄 process.memoryUsage（含 heapUsed／RSS）。只在顯式診斷模式呼叫 GC；一般測試不依賴它。基準與修後採相同 seed=777001、每 seed120步；腳本、原始 log 與10-seed heap snapshot 保留 scratchpad/r44／本機暫存，不進 repo。
- **搜尋效能**：保留300則及50ms門檻，以六種查詢各七次的中位數逐一驗收，包含 input事件、匹配與同步重畫；每次仍驗結果數。不用全查詢共用中位數或單次最佳值掩蓋慢查詢。單次最大值另印，讓GC／CPU排程的偶發停頓可見。歷史332ms樣本沒有 profile，不能斷言那一次必定是GC；本輪不改產品搜尋邏輯，也不放寬門檻。

- **驗收結果**：p3／p5 修前重現；三條新增回歸先紅後綠，npm test **516/516**。指定 `NEWS_WALK_SEEDS=1500 NEWS_WALK_STEPS=120 NEWS_WALK_SEED=777001` 原始命令及加 GC 診斷各完成 **1500 seeds／180000步，1501/1501 tests**，無 OOM／行為斷言失敗。GC後 heapUsed 如下；殘餘小幅變化包含 test runner／V8 開銷，不呈舊版每 seed 約3.6 MiB的線性增長。修後1500點 RSS 約437 MiB，無提高 heap 上限。搜尋六種查詢最大中位數20.67ms，單次最大25.43ms（同時有長漫步負載）；50ms門檻未放寬。

| 完成 seeds | 舊 harness heapUsed（MiB，GC後） | close 後 heapUsed（MiB，GC後） |
|---|---:|---:|
| 100 | 389.0 | 29.1 |
| 400 | 1481.1 | 29.5 |
| 1000 | 未跑 | 31.7 |
...
### 20.45 第 45 輪：v0.7.0 升版與穩定性收尾（jev 0）

- **版本**：modudock.json 從0.6.0升至0.7.0；README 現行閱讀工具段落與本文件 §19 標題同步，保留 v0.2～v0.6 功能首次加入的歷史記錄。此輪另修下述清單語意，不 commit／push。
- **驗證矩陣**：後半以 `/usr/local/bin/python3 -m unittest` 連跑三次，再三份並行一次；前半 `npm test` 連跑三次，再兩份並行一次；`/usr/local/bin/python3 -m unittest tests.test_model_chaos` 六份並行、共五批。各階段分開執行，保留每份完整 stdout／stderr 與退出碼；不提高 timeout／效能門檻或減弱既有斷言。
- **固定負載**：清除外部 NEWS_CHAOS_*／NEWS_WALK_* 覆寫，使用測試預設：chaos 每份60個情境、起始seed47000；前半含200 seeds×100步漫步。額外chaos30份共1800個情境。所有模型呼叫使用假 client／本機假 server，不呼叫真 jev。
- **重跑資料**：scratchpad/r45/stability.py、results.json 與逐份 log；腳本不進 repo。

- **追加無障礙修正**：清單內的閱讀分隔線維持 `li.nw-divider` 與「以下為上次離開前的新聞」文字，移除 role=separator，保留原生 listitem 語意，避免 ul 直接子元素違反 axe list 規則。時間順序的 `li.nw-date-divider` 原本就沒有覆寫 role，保持不變。新增兩種分隔的測試，斷言 nw-list 每個直接子元素都是無role或role=listitem的li，且j/k往返只走新聞標題、跳過分隔。閱讀分隔測試先紅後綠；既有要求separator的斷言依本輪新規則更新，無修改鍵盤導覽程式。

- **結果**：後半六份每份384 tests／OK（連跑三次＋三路並行），前半五份每份518/518（連跑三次＋兩路並行，均含本輪分隔線修正）；chaos六並行×五批共30/30成功、1800個情境。共41個測試程序退出碼皆0，未發現flaky，無提高門檻或更改排程來重試失敗。逐份耗時：後半40.38～41.40秒、前半23.39～33.79秒、額外chaos8.43～9.28秒；git diff --check通過。


### 20.46 第 46 輪：暫時失敗批次同輪補試一次

- **適用失敗**：_ChoiceClient 保留現有對外 failure 代碼，以 request-local ContextVar 另帶內部可補試標記。response（JSON／格式／驗證失敗）、connection（網路／socket／讀取總逾時）、other 中的 HTTP 500～599（不含529）可補試；不把所有 other 一律重送。429／529退避耗盡的busy、401永久auth、403 service、其他4xx、前置拒絕、未預期worker例外不補試。半開熔斷的單次探測亦不補試，維持每輪最多一次HTTP探測及30分鐘冷卻。API題目、採納門檻、原classify_round等獨立client介面不變。
- **排程與身分**：失敗批次保留原work、完整batch及analysis kind，放進cv保護的延後佇列，不產生中間結果、不釋放in-flight key、不先記終局失敗。所有一般可准入lane及其在途請求、未接收結果優先完成，再選延後批次；仍被配對依賴擋住的話題不會阻塞配對補試。一般lane優先序不變；延後批次間亦按分類>配對>話題>分析>基調，且重用三條worker，可三批補試並行。補試成功產生的新一般工作仍優先於尚未准入的補試。
- **一次與預算**：每批只保留一次補試資格，不拆批、不混進其他新批次。第二次批次呼叫失敗即終局failed；原有429／529 HTTP退避仍在該次呼叫內最多兩次，與批次補試分開計。使用原work deadline（首次准入起60秒），不借新輪預算；到期、auth停用或本work已因另一批終局失敗時，釋放延後批次並照舊回傳完成通知，不開始HTTP。原始HTTP失敗時已過期限則沿用原本失敗判斷；已排延後批次等待時才逾期且無其他終局失敗，呈paused/budget。新輪接手舊輪未完成key的原規則保留。
- **資源與熔斷**：延後佇列最多MAX_ITEMS_LIST（300）個完整批次；滿時保留原本終局失敗語意，不丟失in-flight清理。原結果佇列32、各lane有界佇列與cache上限不變。service錯誤不進補試佇列；若一般補試收到403，只按原round id記一次服務失敗，不建立新輪序。工作清空判斷納入延後佇列，避免先印統計或漏收尾；running／awaiting每次仍成對計數。
- **統計**：stderr `model round=` 保留requests批次欄位與http／retries／total_http／total_retries，行尾追加 `requeued=N`。requeued只計實際准入的補試批次；排入後因預算／停用取消者為0。補試另加requests；實際HTTP照常加http，只有429／529的呼叫內重試加retries。例如500→補試429→529→200為requests=2、http=4、retries=2、requeued=1、failed=0。重試成功不保留先前失敗代碼；兩次都失敗才計一個終局failed。real_run摘要加總各完成輪requeued，舊版缺欄則unknown；不新增body.model欄位，因此封包預留不變。
- **測試與既有斷言**：新增14條，含500→200、bad→bad、busy／401／403／4xx不補試、連線恢復、三路原請求與三路補試並行、事件依賴／低lane先做、原輪統計與deadline、排後預算耗盡、半開探測、20則首輪歸零且不送paused／不重publish、退避與補試分列、補試403只計一輪。舊「首個500就停輪」測試改為兩次500才終局或驗證同輪恢復；原快取、釋放、刷新接手斷言保留。耐久測試加延後佇列上限及閒置歸零檢查。新增核心測試先紅後綠；後半全套398 tests／OK，chaos六並行×五批30/30（1800情境）通過。
- **真實驗證**：執行前預告35～45次HTTP，scratchpad/r46/guard 在實際HTTP邊界設50次硬上限；只跑一次現行13來源，未修改feeds.json。實際requests=40、http=40、retries=0、requeued=0、failed=0，獨立邊界計數器亦為40。300則全分類、241則分析、266事件，所有pending歸零、兩個話題；first_list=2.07s、all_done=7.34s、模型elapsed=5.2s。正常路徑無額外補試。保存scratchpad/r46/real.json、real.log、http.json與離線逐份log；不把live key或回應錯誤內文寫入程式／統計。

## 21. v0.8 增補：綜合媒體、來源名額與自由時報保留

本節取代 §19.1／§13.2／§18.1／§18.50 的列表名額及自由時報快取規則；其他來源、API 問法、模型門檻與 60 秒預算不變。版本發布由 cc 審查與真實驗證後另行處理。

### 21.1 新增來源

在原有 13 來源之後追加三個來源，共16來源，保留原 feeds 順序與跨來源去重優先權：

| name | url | outlet | link_domains |
|---|---|---|---|
| ETtoday 即時 | `https://feeds.feedburner.com/ettoday/realtime` | 未填＝name | `["ettoday.net"]` |
| 新頭殼 | `https://newtalk.tw/rss/all` | 未填＝name | 未填，依設定 URL |
| RFI 中文 | `https://www.rfi.fr/tw/rss` | 未填＝name | `["rfi.fr"]` |

2026-09-27 02:47（台灣）以模組 Fetcher 實抓，ETtoday 50、新頭殼 100、RFI 30 則；RFI 文章主機為 www.rfi.fr。聯合新聞網 HTTP 200，但 XML 各 item 的 title/link 皆空，可解析 0 則；再次抓取仍相同，屬來源內容問題，未改 URL 或把空 item 當新聞。HTTP 成功且合法空 feed 的既有 ok 語意不改。

**2026-09-27 聯合 RSS 全空，暫不加入，恢復後可加回（outlet 聯合報）**。cc 另驗6638／7225／6639／6649等頻道，標題與連結皆空、日期1970，故從正式feeds.json移除，避免每輪抓取永遠0則的來源。候選URL保留供恢復後核對：`https://udn.com/rssfeed/news/2/6638?ch=news`。

### 21.2 550 則、保底 20 與封包限制

- 經 cc 選定 `MAX_ITEMS_LIST=550`、`MIN_ITEMS_SOURCE=20`；去重後每 feed 保留最新 min(20, 可用數, 該來源上限)，其餘依發布時間填入。一般來源最多 60 則；自由時報按其 max_items=120。最後依原規則排序，新聞的代表／話題門檻不改。若自訂來源的保底總數超過總名額，仍依 feeds 順序取，保底不能突破列表／大小限制。
- 分類／分析／話題／基調佇列與延後補試上限共用 550，配對佇列為 1100；既有 cache 大小不變。event_size 預留位數改由 `len(str(MAX_ITEMS_LIST))` 推得（目前仍三位），topics 計數預留亦由常數推得。
- **不提高 MAX_PACKET**：`feedparse.MAX_PACKET=900*1024=921600 bytes`，Outbox 共用同一常數；包含完整 envelope、ensure_ascii 序列化與換行。殼實作 `shell/proc/proc.go` DefaultMaxLine、`shell/ws/ws.go` MaxMessageSize、`web/src/protocol.ts` MAX_MESSAGE_BYTES 均為 `1<<20=1048576 bytes`，模組保留餘裕、不改殼。
- 原有 category／analysis／event_size／topic／tone／五個話題與 model 預留維持；超限仍從尾端裁切並重算可見計數。**550 是名額上限，不保證 550 則滿欄位能同時裝入**，保底亦不能保證通過極端大小裁切。
- 回歸測試建立 550 則、17 來源、5 話題，字串取最長允許長度且含非 BMP（連 URL path 也以非 BMP 填滿），計數取保守最大形狀。原始 17,365,903 bytes，fit 後 26 則／904,353 bytes，Outbox 相同；另驗首次空分類／分析預留，補齊後不再減少已送項目。

### 21.3 每來源選填保留規則

- `retain_hours`：可選數值，`0 < retain_hours <= 24`，拒絕 bool、NaN、無窮值與字串。`max_items`：有 retain_hours 時才可設定的 1～120 整數（拒絕 bool），未填用一般來源上限 60。preflight 驗證，無此設定者行為不變。正式只對自由時報設 `retain_hours:3, max_items:120`。
- **當前 RSS 原有報導不因三小時期限消失**。200 成功解析後，以 dedup key 合併本批與前次 cache；同一本批重複 key 取發布時間較新者，本批版本取代舊 cache 版本。已不在本批 RSS 的歷史項目，僅保留 published ≥ now−3h（含邊界）者。整體按發布時間新到舊，最多 120 則；同時按 dedup key 決定穩定次序。因此「保留當前 RSS」仍受 120 則硬上限，並非無限保留。
- Cache 額外記 `current_keys`：上次成功 RSS 中且仍在有界 cache 的 key，最多 120 個。items、validators、first_seen、current_keys 隨候選整批提交，舊輪或逾期候選仍拒收。worker 只建立候選，不直接改共享 cache。
- 有效 304 沿用當前 RSS 身分及 validators，last_success 照舊更新；失敗／parse error／期限到期則沿用舊 RSS 身分與快取、不更新 last_success。每輪抓取收尾在 cv 內清除已漏列且過期的歷史項目，含 304／失敗輪；不重新賦予 published。沒有新 200 前，上次 RSS 本身的項目仍可留作舊資料，即使超過三小時。正常來源仍整批替換、失敗沿用。
- first_seen 仍為每來源 1000 筆 LRU，獨立於保留 cache；無效／缺日期採初次觀察時間，保留、304、失敗不續期、不把 now 寫成新日期。LRU 淘汰後再出現的無日期項目仍依既有規則重新推定時間，不新增無上限的身分表。
- 僅記憶體保留；停止 process 即消失，首次啟動不能補回 RSS 已漏列的舊文。10 分鐘兩次抓取間就出現又消失的報導仍可能漏收。模型成功快取繼續按 dedup key 共用，新舊列表不重問已快取的分類等答案。

### 21.4 同份快照留存、大小與模型成本

以下保留移除聯合前的量測紀錄（17來源）；正式設定現為16來源。封包回歸仍使用17來源作較保守的壓力情境，不下修既有安全驗證。以同份 17 來源實抓資料比較，聯合新聞網本次為 0，不把合成資料列為真實抓取成果：

| 方案 | 合併則數 | 原13來源原本300則仍可見 |
|---|---:|---:|
| 原13來源，300／保底3 | 300 | 300 |
| 加4來源但維持300／保底3 | 300 | 178 |
| 550／保底20 | 478 | 300 |

- 新方案 Yahoo 50、中央社政治20、BBC42、MoneyDJ20；對照舊17來源方案為22／20／3／3。新增 ETtoday50、新頭殼60、RFI30、聯合0。
- 另做**合成壓力情境**：自由時報補到120則、聯合恢復20則，其餘固定同快照。500／保底20只能保住其他舊12來源原260則中的214則；550可保住255／260（98.1%），Yahoo47、科技新報38，其餘未少於原本數量。這是選550的依據，不代表全天每輪都能維持同一比例。
- 同快照478則，離線裝飾的初始完整列表491,952 bytes；離線將每則補上最長分析形狀、category/topic/tone，並放入五個最大話題，584,604 bytes，**裁切0則**。合成550則同樣填滿模型欄位為656,261 bytes，裁切0則。沒有用真 API 生成本次分類／分析／話題，這是傳輸大小壓力測量，非模型品質驗證。
- 另實跑 `env -u TYPESAFE_API_KEY /usr/local/bin/python3 scripts/real_run.py`：17來源完成，首份列表2.15秒、478則／489,507 bytes、無大小裁切，off／exit0，requests=http=retries=requeued=0。聯合仍為合法空feed。
- 空快取、一般短標題摘要：分類300→550由15→28批；若每則標題300＋摘要200字都滿，受8000字限制可到19→35批。同份實抓快照300→478，分類15→24批，配對候選132→217（自動15→19），依現行 state≤20／題數≤40／8000字貪婪分批，待問配對8→13批。分類＋配對可估23→37次；分析、話題、基調須視分類／配對答案，不能離線聲稱精確總次數。
- 以先前約八成需分析估計，550則約440則分析、約22～24批，加分類28批後約50～52批，再加當輪配對、話題及基調；550則的所有可能配對上界為150975（300則為44850），實際有36h／bigram過濾及快取。成本不宜單純按則數倍增；仍共用原60秒預算、3 worker、熔斷與重試政策，可能分到下一輪完成。**本輪真 jev 請求0**，完整輪成本由 cc 真實驗證。

### 21.5 驗證

- `/usr/local/bin/python3 -m unittest`：408 tests／OK（61.873s）。新增10條保留／配置／封包測試；既有滿額、保底、來源上限、模型佇列與候選補排改依550／20驗證。550則在首輪分類與分析全部歸零，快取命中路徑亦全數完成；滿額測試隔離下一次定時更新（interval=3600），等待上限20秒適應56個模型批次與完整補送，仍斷言round_id=1及零額外publish。歷史300則事件快照本身不改，專注事件的fixture載入排除新加的來源保留設定，避免歷史日期被新TTL移除。
- `npm test`：519／519，沒有修改前半程式。原300則搜尋測試保留，另測550則；六種查詢各七次的中位數最大26.10ms，單次最大29.78ms，仍用50ms門檻。
- 前後量測與RSS快照保存在 `/private/tmp/news-v08/`；未commit、未呼叫真jev、未改殼；版本宣告依cc後續指示升為0.8.0，待cc真實驗證。快照及高流量合成資料不混稱即時模型驗證。

- **移除聯合後重驗**：正式來源改16個、modudock.json升0.8.0；`/usr/local/bin/python3 -m unittest` 408 tests／OK（62.181s），`npm test` 519／519。來源配置測試明確斷言聯合不存在；17來源封包壓力情境保留。未commit，待cc真實驗證。

### 21.6 審查修正：配對候選共用快取

- `_emit` 每份新來源列表產生一次 candidate_pairs；`_decorate_events`、`_enqueue_classification`、`_unresolved_events` 與補送共用同一份 immutable Pair tuple。以物件身分命中已發布列表及一份製作中的列表，最多保留這兩份引用／候選；非自動配對 key 在話題准入需要時才建立，不累積歷輪列表或所有副本。
- 裝飾的 deepcopy 不改 link/title/summary/published，直接承接候選；fit 仍在 cv 外執行，前後比較這四個欄位與順序。裁切／修改／重新排序即重算，未變則承接；送出成功後 last_list 的副本亦承接。若連續接收答案先產生多份副本、最後要送的並非最新製作副本，先核對已發布列表的輸入欄位再共用，不為每份副本維護無上限的身分表。正式新抓取輪及外部替換 last_list 的新身分仍會重算。
- 僅快取候選，不快取 pending、same/different 答案、分群或 unresolved 布林值；每次仍以最新 event_cache 重新判定，含 false、答案更新及FIFO淘汰。自動配對超出20000快取時，仍用候選推得的自動邊分群，不新增待問工作。公開封包不增加欄位，保留原cv邊界及bye政策。
