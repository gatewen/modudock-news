...
**真 API 驗證（19:5x，cc-mod）**：對該話題 41 則問「報導基調」→ 負面 17、中性 15、正負並陳 5、正面 4；逐則人工檢查約 8 成同意，錯誤多為「中性 vs 負面」邊界；彙總方向與實際報導敘事（「排場十足但進展有限」）一致。
...
**R8-B 基調顯示（前半）**
- 焦點區話題列：標題下方一行細條（高 4px，沿用 nw-bar／nw-segment；色段固定順序 正面、正負並陳、中性、負面，同股市訊號條）＋小字「報導基調：負面 17・中性 15・正負並陳 5・正面 4」（依數量多→少、只列 >0 者）。**已判斷則數 ≥5 才顯示**。
- 顏色：負面 `--nw-tone-neg`（正文色 78%，深墨色）、正面 `--nw-accent`、正負並陳 `--nw-mixed`、中性 `--nw-idle`。**不用任何紅或綠**：財經檢視中紅色＝上漲（§18.35 修正：原本負面用 danger 紅，與股市訊號並排時語意相反）。
...
- 前半：**只在話題篩選中**，清單列 meta 加基調小標（「負面」「正面」「正負並陳」「中性」），顏色同焦點基調條（負面 danger、正面 accent、正負並陳 mixed、中性 muted），1px 同色外框、不用實心底；群組代表列顯示代表報導的基調，展開的子報導各自顯示。未篩選話題時不顯示（避免清單雜訊）。
...
- **面板語意**：開啟只看上次之後時，樣本列開頭加「上次離開後的新進展：」，明示統計已縮至新進展；關閉移除前綴。搜尋不縮面板，新進展會縮；兩者並用時，下述搜尋提示與樣本列前綴同時顯示，來源／類別下拉仍維持原分面範圍（§20.11）。搜尋有字時，樣本列前顯示「統計為全部{類別名}，未套用搜尋」，清除即隱藏；面板仍沿用來源／類別／話題範圍，題材／數字不另加提示。股市訊號旁註「依新聞內容判斷對股市的影響，非行情」，四段新詞同步按鈕、篩選說明、aria、title 與歷史。大盤行為「大盤方向：」後依序為「大盤／總經 N 個事件」「利多 X」「利空 Y」三個按鈕，以 gap 分隔、不加浮動符號（§20.9），三個計數仍可點；其後另加非按鈕小字「其餘 N 件無明確方向」，N=總數−利多−利空，0 時不顯示（§20.23）。財經題材排行圖例「紅＝偏多・綠＝偏空・灰＝無方向」；國際使用升級／緩和／無方向及相應 danger／accent／idle 色，政治不顯示方向圖例。清單財經題材 ▲▼ 可見字不改，title 與外部 aria-describedby 說明該新聞對題材偏多／偏空（非行情），無方向則為「題材：{題材}」。報導基調的正面／負面／正負並陳／中性不改。（出處：§20.8）
...
- **話題內閱讀工具**：只在話題及話題內數字篩選顯示「最新更新｜時間順序」二選一分段按鈕（role=group、各自 aria-pressed；§20.33）；預設沿用最新優先，順讀按符合條件事件最早報導時間由舊到新，加實際 M/D 日期分隔，title 明示「依發布時間，非事件發生時間」。時間順序顯示「由舊到新，依發布時間」；順讀不放上次離開分隔線，新進展計數保留；j/k 按實際列順序。離開或切換另一話題重設，補送保留，不持久化。來源分布升級為「各家基調對照（則）」：按 outlet 合併 feed，每家一列媒體按鈕、報導數、基調比例條與短數字，前五家外以「另 N 家（共 M 家）」按鈕展開。比例只含有效逐則 tone，未判定另列；順序、圖例與焦點一致（§20.32）。按媒體篩該話題，來源下拉顯全部來源，表格旁顯「只看：媒體名 ✕」chip，✕ 無障礙名稱為「清除媒體篩選」，排序鈕獨立；同一家再按或清除只取消媒體。來源下拉手動變更沿用退出話題規則，返回恢復進入前檢視。分布數字為套用類別、題材／數字、搜尋、追蹤、新進展後可列出的報導數，不受已選 outlet 限制；數字＝點後報導數，同事件可含多則。分布 title 列全部，總家數仍按完整話題 outlet 去重。窄版換行、不縮寫。（出處：§20.26）
...
- **計數與範圍**：首頁焦點話題列，以及進入話題篩選後仍顯示的同一話題列，共用基調入口；不另複製第二組基調條。沿用至少五則有效基調才畫條的門檻、固定色段順序及文字按數量排序。每個非零計數為原生 button，§20.23 改可見文字為「負面／中性／正負並陳／正面 N」，行首「報導基調（則）：」，aria-label 為完整「名稱 N 則報導」且包含可見文字；aria-expanded 與 aria-pressed 同步。
...
- **R23-4 基調短字**：mixed 的前半名稱統一為「正負並陳」，含焦點、核對區、清單與子報導小標。行首為「報導基調（則）：」，按鈕可見「負面 11」這樣的短式，數字仍加粗；aria-label 保留「負面 11 則報導」，完整包含可見字，維持 label-in-name。核對區內重述仍用含單位的完整名稱。id、排序、配色、逐則計數與後半驗證過的 criteria 原文不改（§18.8 的 API mixed「正反並陳」是模型文字，不是 UI 名稱）。
...
- **對照表**：話題來源分布改為「各家基調對照（則）」，每列一個 outlet，合併同媒體各 feed。媒體名與報導數為同一按鈕，沿用點選／再點取消的媒體篩選。依報導數由多至少、同數依來源順序，預設前五家，其餘以「另 N 家（共 M 家）」展開，aria-expanded 與 aria-controls 對應；展開後可收合。同話題補送與篩選保留展開，退出／換話題重設，不持久化。
- **統計範圍**：沿用 §20.26，各家數字套用類別、題材／數字、搜尋、追蹤、新進展，但不受所選 outlet 限制。因此各家則數等於點後報導數，並非摺疊事件數。有效 item.tone 按負面／中性／正負並陳／正面計數，缺失或非法值另列「待判定 N」，不進比例分母；沒有已判定報導時隱藏比例條。短數字為負／中／正負／正，零不顯示，title 保留完整名稱及報導單位。各短數字合計等於該媒體已判定報導數；對照不另設五則門檻，焦點整體基調既有五則門檻不變。
- **呈現一致**：焦點、話題基調及對照表的色段、計數、圖例統一固定負面→中性→正負並陳→正面（取代先前色段順序與按數量排序的文字）；沿用既有深墨／灰／琥珀／accent token，數字前同色小方塊 aria-hidden。基調核對區仍按整個話題，不隨清單篩選；對照表按上一條可列出報導範圍。
...
- **R33-3**：「報導基調（則）」與「各家基調對照（則）」title 說明：依標題與摘要判斷報導的語氣，正面＝強調成果、進展、合作或利多；負面＝強調分歧、受挫、風險、抗議或批評；中性＝主要陳述事實、行程或背景；正負並陳＝正反並陳；非媒體立場。依 back/topics.py TONE_CRITERIA，不改模型題目。話題清單同事件符合條件報導有多種有效 tone 時，小標顯「負 N・中 N・正負 N・正 N」（零不列），不只取代表；非法／未判定不混入，子報導仍個別標示。
