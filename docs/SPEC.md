...
- **claimed／身分**：claimed 只記實際成員 key，候選及 sticky seed 排除都按 key。每個種子只直接納入其事件中未 claimed 的報導，話題間成員不重複。新種子仍須剩餘至少三家，取剩餘最早（同時依來源順序、key）為代表；未被 claimed 的精確 previous seed 沿用 id，擴展後仍須三家。false 僅作用於該 seed→key，不封鎖另一話題判斷；上述大事件整組納入為刻意例外。
