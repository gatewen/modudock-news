...
- **話題內閱讀工具**：只在話題及話題內數字篩選顯示固定文字「時間順讀」切換（aria-pressed=true 為順讀，false 為最新優先，title 說明下一個動作；§20.27）；預設沿用最新優先，順讀按符合條件事件最早報導時間由舊到新，加 M/D／今天日期分隔，title 明示「依發布時間，非事件發生時間」。順讀不放上次離開分隔線，新進展計數保留；j/k 按實際列順序。離開或切換另一話題重設，補送保留，不持久化。來源分布前五家改可按，按 outlet 合併 feed 篩該話題，來源下拉顯全部來源、另列「話題內・媒體名」及清除；同一家再按或清除只取消媒體。來源下拉手動變更沿用退出話題規則，返回恢復進入前檢視。分布數字為套用類別、題材／數字、搜尋、追蹤、新進展後可列出的報導數，不受已選 outlet 限制；數字＝點後報導數，同事件可含多則。分布 title 列全部，總家數仍按完整話題 outlet 去重。窄版換行、不縮寫。（出處：§20.26）

- **補送閱讀位置**：只在接收同／新 at list 時，記錄最近可捲動祖先視窗內第一個可見報導連結的身分與 y 位置，重畫後補償 scrollTop；原列消失則依序找原視窗內下一個仍存在的報導，全部消失不強制定位。被合併成子報導時展開該群並定位原報導。主動搜尋／篩選／進出話題不套用，自動話題返回沿用原返回位置；鍵盤閱讀沿用原焦點規則，pointer／wheel 後恢復滑鼠位置保護。模組根 overflow-anchor:none，只停用模組自身原生 anchoring，避免與手動補償重複。未更改殼樣式。（出處：§20.28）
