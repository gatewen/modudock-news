...
- **配對完成前的話題**：events.pending 尚未歸零時不重新 plan 話題，也不開始新的話題請求；保留上次話題，依本份列表仍存在的成員重算數量，仍須至少三家不同 outlet 才保留。配對完成才更新話題；種子若被裁掉，不再保留黏著身分。跨輪已排入的舊話題工作目前有准入缺口，見 §20.18 P3；「配對未完不開新話題請求」仍是規格要求，未於本輪改寬。（出處：§18.7、§18.9、§18.12、§18.30、§18.37）
...
- **結構與樣式**：`front.js` 管 mount／狀態／DOM，`style.js` 匯出 scoped CSS，`labels.js` 放固定名稱與無狀態驗證。style 放模組根 section.nw，CSS 不操作殼或 document.head；根 `.nw` 設 position:relative，讓 `.nw-sr` 與窄版快捷鍵隱藏名稱的 absolute 定位留在模組內；隱藏文字使用 1px、clip／clip-path、overflow:hidden、white-space:nowrap，不用 display:none（§20.17）。吃 --md-* token、light-dark 深淺色、container query 560／420 斷點。工具列、篩選與列文字同左緣；財經紅漲綠跌，國際升級用 danger、緩和用 accent。基調負面用深墨色／深色亮灰，不借用財經紅綠。（出處：§14、§15、§17.3、§18.22、§18.27、§18.35）
...
- **焦點入口**：優先最多五個話題；未處於 selectedTopic 話題檢視時，來源＋類別只決定話題是否有符合成員；話題檢視中保留所有話題入口，內部切類別不隱藏這些入口。題材／數字／搜尋／新進展不篩話題卡，話題家數、基調與新增數仍按整個話題。被過濾光不回退事件焦點。寬版「看話題・N 家」、窄版「N 家」進話題，標題連結仍開原文；話題 title、最新不同事件的報導標題、基調、上次之後新增事件數依資料呈現。最新列與種子同標題或同事件即不重複列，長標題單行省略、title 留全文。（出處：§18.6、§18.20、§18.23、§18.39、§18.43）
- **焦點回退與基調**：無話題且 working 時顯示「正在整理多家媒體同報的話題」；其他無話題情況，依目前清單篩選找至少三家報導的事件，按家數、最新時間、id 取五個，「看同事件・N 家」展開並定位該事件。若仍為零，僅在無篩選且全部資料也無三家事件時改顯緊湊一行「目前沒有多家媒體同時報導的新聞」，無卡片底色與上下內距，隱藏標題；title 保留至少三家不同媒體門檻（§20.9）；有篩選而無可顯示事件則隱藏（§20.6）。只看追蹤仍隱藏焦點，有話題但被來源／類別篩光也仍隱藏。收到的 topic.tone 必須先是合法非負整數計數、合計不超過 topic.count，否則整個基調區不顯示。通過此檢查後，話題至少五則有效逐則基調才畫基調條與計數按鈕；色段固定正面／正反／中性／負面，文字計數按數量排序，首頁保留可見。前半以目前話題逐則有效 tone 重算可核對計數，缺失或非法 tone 另列「待判定 N 則」，未滿五則也可顯示待判定。計數文字改為原生按鈕，如「負面 3 則報導」，可見文字即 accessible name；按鈕無底色與外框、數字加粗，hover 底線、選取時粗底線、保留 focus-visible 外框。在首頁與進入話題後都可使用（§20.12）。（出處：§18.3、§18.8、§18.20、§18.35、§18.43、§20.1）
- **基調核對區**：點基調按鈕，在該話題列下方展開「依標題與摘要判斷的報導基調・按報導計」，逐則列出貢獻該數字的標題連結、來源、時間及可展開來源摘要；同事件不摺疊，不按清單類別／搜尋／追蹤／新進展篩選縮減話題計數。同時只開一個，再點同鈕或模組內非編輯區／搜尋輸入框的 Esc 收起並回焦點，Esc 順序依 §20.15 優先於搜尋清除；其他輸入區不攔。補送重算、按話題＋基調保留選擇；話題／該基調按鈕消失或話題被藏起即收起，之後不自動復開。摘要沿用清單元件，以來源＋連結＋標題識別單篇，保留到報導離開快照或 unmount，不持久化（§20.12）。
- **分析面板範圍**：finance／tech 顯示財經、world 顯示國際、politics 顯示政治，其他類別或只看追蹤時不顯示。範圍是來源＋類別＋當前話題限制後的報導；若開啟只看上次之後，再保留含較新報導的完整事件（§20.11），不受題材／地區／議題或數字篩選影響；每事件取時間排序中第一則有效分析。數字篩選若從話題內啟用，面板與清單保留該話題範圍。（數字篩選見 §20.1）樣本列為 N 個事件（M 則報導），K 個來源，N<10 提示「樣本少，僅供參考」；樣本另列「已分析 X／N」；缺有效分析的事件不進方向條或四個按鈕，有缺額且分類啟用、model 非 paused 時顯示「待判定 Y」。off／paused 沿用狀態列的分類關閉／整理暫停文字，不把缺額當無關；待合併取 body.events.pending。（§20.11）（出處：§16.6、§18.2、§18.11、§18.16、§18.43）
...
- **本輪待審的程式落差**：追蹤計數漏套搜尋／部分新進展交集、其他地區／議題最多時的長條縮放、跨輪舊話題佇列的配對准入限制，已重現並列於 §20.18 P1～P3。本輪只更正文檔，未修改程式，也未把這些落差改成新規則。
