...
- 每列：時段標籤｜細條（高 6px，色段與順序同上方主訊號條）｜右側數字：財經為「正面 N%」（positive ÷ (positive+negative+mixed)，四捨五入整數），國際為「升級 N%」（escalation ÷ (escalation+stalemate+deescalation)）。分母為 0 時顯示「—」；分母 1～4 時不換算百分比，改顯示「升級 3/3」這種計數（小分母的百分比會誇大）。窄版標籤欄固定 6ch、寬版 11ch，細條起點對齊。
