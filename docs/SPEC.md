...
- **閱讀分隔線**：lastSeen 是 mount 時凍結的時間基準，不是逐則已讀紀錄。事件任一報導較新就算新事件；清單開頭連續新事件後若還有舊事件，插入「以下為上次離開前的新聞」。分隔線與狀態新增數均按目前篩選後的群組計算。全新／全舊／第一群就是舊的不插入；全部為新事件時，各主列也不標「新」，清單的新舊提示只靠狀態列新增數；展開子報導平常不標「新」；開啟「只看上次之後」時，lastSeen 後發布的子報導加「新」（§20.11）。非開頭連續區段的新事件仍加「新」，焦點亦保留。狀態顯示可切換的「新增 N 個事件」按鈕，N=0 或無 lastSeen 時隱藏且禁用，不顯示新增文字與其前方分隔符；aria-pressed 同步，按鈕節點保留以維持焦點。開啟後只列符合目前條件、且至少一則報導在 lastSeen 後的事件，保留其較舊代表；與來源／類別／題材／數字／話題／搜尋／追蹤取交集。清單前提示「只看上次離開後的新進展（N 個事件）」及獨立關閉鈕，即使 N=0 仍可退出。面板、24 小時分段也縮至新進展事件，選單分面數維持原範圍；搜尋仍不縮面板。補送保留、清除全部取消、話題返回還原進入前開關，不持久化（§20.11）。話題顯示「上次之後新增 N 個事件」；首次無基準則都不顯示。（出處：§18.4、§18.12、§18.13、§18.20、§18.43）
