...
- **窄版面板與工具列**：ResizeObserver 以模組根內容寬度 ≤480px 判窄版，不以 viewport 判斷。面板預設收合為摘要按鈕，財經／科技顯偏多與偏空、國際顯升級與緩和、政治顯事件數；缺分析才附已分析 X／N。aria-expanded／aria-controls 對應面板內容；展開選擇跨類別與補送保留，只在本次掛載記憶，不寫本機儲存；寬版始終顯示完整面板，縮回窄版沿用選擇。尺寸收合時若焦點在內容，移回摘要鈕；切寬版若焦點在摘要鈕，移至清單。寬版工具列 DOM／Tab 順序為重新整理→來源→類別→追蹤關鍵字→只看追蹤（若有）→搜尋；窄版搬動既有節點為來源→類別→主操作行，視覺與 Tab 順序一致，跨門檻保留焦點，不以 CSS order 排序（§20.25）。窄版四個訊號鈕為 2×2、不斷詞；下拉並排第一行，重新整理／追蹤關鍵字／只看追蹤／搜尋在第二行，極窄時該操作行可橫捲，不多折一行；狀態、速覽與展開的輸入區另列。（出處：§20.24）
...
- **話題內閱讀工具**：只在話題及話題內數字篩選顯示「最新優先／時間順讀」切換（aria-pressed）；預設沿用最新優先，順讀按符合條件事件最早報導時間由舊到新，加 M/D／今天日期分隔，title 明示「依發布時間，非事件發生時間」。順讀不放上次離開分隔線，新進展計數保留；j/k 按實際列順序。離開或切換另一話題重設，補送保留，不持久化。來源分布前五家改可按，按 outlet 合併 feed 篩該話題，來源下拉顯全部來源、另列「話題內・媒體名」及清除；同一家再按或清除只取消媒體。來源下拉手動變更沿用退出話題規則，返回恢復進入前檢視。分布數字為套用類別、題材／數字、搜尋、追蹤、新進展後可列出的報導數，不受已選 outlet 限制；數字＝點後報導數，同事件可含多則。分布 title 列全部，總家數仍按完整話題 outlet 去重。窄版換行、不縮寫。（出處：§20.26）
