...
class Scheduler:
    def __init__(self, feeds, fetcher, outbox, seq, *, interval=600,
        ...
        self.model_requeue_running = 0
        self.model_requeues = deque()  # Whole batches; retain in-flight ownership until final acknowledgement.
        self.total_http = 0
    ...
    def _next_requeue(self):
        # Normal lanes (including work generated by accepted results) go first.
        # Wait for original in-flight batches and unaccepted completions; retries
        # themselves may overlap, up to the existing three worker threads.
        if (self.model_running > self.model_requeue_running or self.results
                or self._next_lane() is not None
                or (getattr(self.classifier, 'service_circuit_open', False) and self.model_running)):
            return None
        for lane in self.lanes:
            if lane.name == 'topics' and self.event_in_flight:
                continue
            for index, entry in enumerate(self.model_requeues):
                if entry[0] is lane:
                    return index
        return None
        ...
        while True:
            with self.cv:
                ...
                if allowed:
                    ...
                    work.requeued += int(requeued)
                    work.running += 1
                    self.model_running += 1
                    self.model_requeue_running += int(requeued)
                    if not work.logged:
            ...
            if allowed:
                ...
                try:
                    ...
                    retryable = _retryable_failure.get()
                    _retryable_failure.reset(retry_token)
            ...
            with self.cv:
                if allowed:
                    ...
                    self.model_running -= 1
                    self.model_requeue_running -= int(requeued)
                if (allowed and result is None and retryable and not requeued
                        and failure_detail not in ('busy', 'service')
                        and not work.failed and self._classify_enabled()
                        and self.model_clock() < work.deadline
                        and len(self.model_requeues) < MAX_ITEMS_LIST):
                    # No result yet: do not release keys, count a terminal failure,
                    # or let round statistics close before the one final attempt.
                    self.model_requeues.append((lane, work, batch, kind))
                    self.cv.notify_all()
                    continue
